...
function GenerateHeightmap ()
{
	...
	for (y=0;y<height;y++)
	{
		for (x=0;x<width;x++)
		{
			...
			uv[idx] = Vector2.Scale(Vector2 (x, y), uvScale);
			colors[idx] = Color32(0, 255, 0, 255);
			...
			tangents[idx] = Vector4( tan.x, tan.y, tan.z, -1.0 );
		}
	}
...
function Update()
{
	...
	for (y=0;y<height;y++)
	{
		for (x=0;x<width;x++)
		{
			...
			var idx : int = y*width+x;
			var differenceX = x - Input.mousePosition.x;
			var differenceY = y - Input.mousePosition.y;
			var pixelHeight = Mathf.Cos(differenceX * 0.1f);
			var vertex = Vector3 (x, pixelHeight, y);			
			vertices[idx] = Vector3.Scale(sizeScale, vertex);
			colors[idx] = Color32(255, 255, 0, 255);
